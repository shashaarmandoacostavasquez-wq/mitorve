...
function requireConfig(res) {
  if (!adminClient) {
    res.status(500).json({ error: 'Servidor sin configurar' });
    return false;
  }
  return true;
}
...
app.get('/api/products', async (req, res) => {
  ...
  const { data, error } = await adminClient
    .from('products')
    .select('id,name,slug,category,description,price,image_url,active')
    .eq('active', true)
    .order('name');
...
app.post('/api/orders', async (req, res) => {
  ...
  if (pErr) {
    console.error('PRODUCT VALIDATION ERROR:', pErr);
  ...
  for (const x of items) {
    ...
    safeItems.push({
      ...
      quantity: qty,
      customization:
        String(x.customization || '').slice(0, 500) || null
    });
  }
  ...
  const { data: order, error: oErr } = await adminClient
    ...
    .insert({
      ...
      delivery_address: address || 'A coordinar',
      delivery_notes:
        String(b.delivery_notes || '').slice(0, 1000) || null,
      payment_method: 'cash',
      subtotal,
  ...
  if (iErr) {
    console.error('CREATE ORDER ITEMS ERROR:', iErr);
    ...
    return res.status(500).json({
      error: 'No se pudo guardar el detalle del pedido'
    });
  }
...
app.listen(port, '0.0.0.0', () => {
  console.log(`MITORVE escuchando en ${port}`);
});
